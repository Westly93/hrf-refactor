 ...
 */
  ...
  const handleSubmit = async (event) => {
    ...
    if (file.size > 1024 * 1024 * 20) {
      setErrorMessage("File size cannot exceed 20MB");
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      // console.log("app response", applicationResponse)
      ...
      if (applicationResponse.error) {
        ...
        setApplicationID(applicationResponse.payload.id)
        ...
        for (const req of experience) {
          ...
          if (requ.error) {
            const { message, errors: payloadErrors } = requ.payload;
            setErrorMessage(payloadErrors);
            setIsLoading(false);
            return;
          }
          ...
          // console.log("apsc", appscores)

          if (appscores.error) {
            const { message, errors: payloadErrors } = appscores.payload;
            setErrorMessage(payloadErrors);
            setIsLoading(false);
            return;
            // setIspublished(false);
        ...
        const appdocs = await dispatch(
          applicantDocuments({
            application_id: applicationResponse.payload.id,
            document_type_id: 1,
            file: file,
          }))
        // console.log("application documents", appdocs)

        if (appdocs.error) {
          setApplicationErrors(appdocs.payload.errors);
          // setErrorMessage(appdocs.payload.errors);
          setIsLoading(false);
          return;
        }
          ...
          <div className="row mb-4">
            ...
            {applicationErrors && (
              ...
              <div className="alert alert-danger fw-bold">
                ...
                There was an error uploading your application document. Please use the
                <Link to={`/upload-application-document/${applicationID}`} className="alert-link"> link</Link> to upload your document.
