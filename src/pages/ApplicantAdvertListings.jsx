...
const ApplicantAdvertListings=() =>{
  ...
  console.log('compare two dates', (new Date(formattedDate) <= new Date(adverts[0]?.closing_date)));
  console.log(adverts[0]?.closing_date)
  ...
  // console.log('not expired adverts ', data[0]?.closing_date)
  ...
  const lastIndex = currentPage * dataPerPage;
  const firstIndex = lastIndex -  dataPerPage;
  const validAdverts= data?.filter(advert=> (new Date(advert.closing_date)>= new Date(formattedDate)))
  const currentData = validAdverts?.slice(firstIndex, lastIndex);
