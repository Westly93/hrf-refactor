      ...
      if (applicationResponse.error) {
        ...
        for (const document of selectedDocuments) {
          ...
          if (file === undefined) {
            ...
            setErrorMessage("File is missing for the selected document.");
            continue;
          }
          ...
          formData.append("applicant_id", user?.user?.profile?.id);
          await dispatch(applicantDocumentsAdded(formData));
          console.log("applicant docs", formData);
        }
...
{/* The section for application documents */}
            ...
            <div className="mb-3">
                ...
                >
                  ...
                  {/* {Array.isArray(docTyps) &&
                    ...
                    {Array.isArray(docTyps) &&
                      docTyps
                        .filter(type => type.name === 'Curriculum Vitae')
                        .map((type) => (
                          <option key={type.id} value={`${type.id}:${type.name}`}>
                            {type.name}
                          </option>
      ...
      {documentSizeErr && <span className="text-danger">{documentSizeErr}</span>}
